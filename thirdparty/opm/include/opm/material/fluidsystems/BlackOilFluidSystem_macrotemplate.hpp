...
// 采用 C++17 内联变量，避免 ODR 冲突。
// 初始化值与 initBegin() 与现有 .cpp 定义保持一致。
#define DECLARE_INSTANCE(T)                                                                                            \
    template <>                                                                                                        \
    inline unsigned char BOFS<T>::numActivePhases_ = 0;                                                                \
    template <>                                                                                                        \
    inline std::array<bool, BOFS<T>::numPhases> BOFS<T>::phaseIsActive_ = {false, false, false};                       \
    template <>                                                                                                        \
    inline std::array<short, BOFS<T>::numPhases> BOFS<T>::activeToCanonicalPhaseIdx_ = {0, 1, 2};                      \
    template <>                                                                                                        \
    inline std::array<short, BOFS<T>::numPhases> BOFS<T>::canonicalToActivePhaseIdx_ = {0, 1, 2};                      \
    template <>                                                                                                        \
    inline T BOFS<T>::surfaceTemperature = T(0);                                                                       \
    template <>                                                                                                        \
    inline T BOFS<T>::surfacePressure = T(0);                                                                          \
    template <>                                                                                                        \
    inline T BOFS<T>::reservoirTemperature_ = T(0);                                                                    \
    template <>                                                                                                        \
    inline bool BOFS<T>::enableDissolvedGas_ = true;                                                                   \
    template <>                                                                                                        \
    inline bool BOFS<T>::enableDissolvedGasInWater_ = false;                                                           \
    template <>                                                                                                        \
    inline bool BOFS<T>::enableVaporizedOil_ = false;                                                                  \
    template <>                                                                                                        \
    inline bool BOFS<T>::enableVaporizedWater_ = false;                                                                \
    template <>                                                                                                        \
    inline bool BOFS<T>::enableDiffusion_ = false;                                                                     \
    template <>                                                                                                        \
    inline std::shared_ptr<OilPvtMultiplexer<T>> BOFS<T>::oilPvt_ = {};                                                \
    template <>                                                                                                        \
    inline std::shared_ptr<GasPvtMultiplexer<T>> BOFS<T>::gasPvt_ = {};                                                \
    template <>                                                                                                        \
    inline std::shared_ptr<WaterPvtMultiplexer<T>> BOFS<T>::waterPvt_ = {};                                            \
    template <>                                                                                                        \
    inline std::vector<std::array<T, 3>> BOFS<T>::referenceDensity_ = {};                                              \
    template <>                                                                                                        \
    inline std::vector<std::array<T, 3>> BOFS<T>::molarMass_ = {};                                                     \
    template <>                                                                                                        \
    inline std::vector<std::array<T, 9>> BOFS<T>::diffusionCoefficients_ = {};                                         \
    template <>                                                                                                        \
    inline bool BOFS<T>::isInitialized_ = false;                                                                       \
    template <>                                                                                                        \
    inline bool BOFS<T>::useSaturatedTables_ = false;                                                                  \
    template <>                                                                                                        \
    inline bool BOFS<T>::enthalpy_eq_energy_ = false;
DECLARE_INSTANCE(float)
DECLARE_INSTANCE(double)
#undef DECLARE_INSTANCE
#endif
